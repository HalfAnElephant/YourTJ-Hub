...
function streamOf(posts: PostPayload[]): PostWindowPayload {
  return { posts, hasBefore: false, hasAfter: false }
}

// post-view-mode 使用模块级共享状态（与 home-feed-mode 同模式），
// 每次挂载前 resetModules 让组件拿到全新模块并重新读取 localStorage。
async function mountPostStream(contentType: 1 | 3, posts: PostPayload[]): Promise<VueWrapper> {
  vi.resetModules()
  ...
  const wrapper = mount(PostStream, {
    props: {
      ...
      topicTitle: '测试话题',
      contentType,
      initialPostStream: streamOf(posts),
      viewer,
      canPost: true,
    },
    global: {
...
describe('楼层流树状/扁平双视图（组件级集成）', () => {
  ...
  test('胶囊切换即时生效并按内容类型持久化，类型间互不影响', async () => {
    ...
    // Q&A（默认树状）切到扁平：#3 回到链根 #2 的堆叠块（article 内 PostReplyRow）
    ...
    expect(qa.find('article[data-post-no="2"]').element.contains(qaRow3.element)).toBe(true)
    ...
    expect(qaReloaded.find('article[data-post-no="2"]').element.contains(qaReloaded.find('[data-post-no="3"]').element)).toBe(true)
  })
